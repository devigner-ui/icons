...
The package ships one executable, `icons`, with no dependencies of its own:
...
and rebuilt as React components:
...
```bash
pnpm build   # the only build; every drawing kept
...
| Path | What it is |
| --- | --- |
| `packages/icons` | The published package. `icons/` holds all four drawings per symbol. |
| `packages/icons-pro` | Licence-handling stub. Unlocks nothing today. |
