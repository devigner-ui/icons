...
8,584 open-source icons for React. Every one of the 2,146 symbols carries four separate drawings:
...
**All four styles are free on all 2,146 symbols.** Bulk used to be sold on 1,682
...
- **Typed.** `IconName`, `IconStyle`, `IconProps` and `IconComponentType` are
  exported; `IconName` is a literal union of all 2,146 symbol names.
