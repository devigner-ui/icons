...
7,852 icons for Vue 3: 1,963 symbols, each in outline, twotone, bold and bulk.

- One module per icon, so a bundler keeps only what you import.
- `variant` picks the drawing; `strokeWidth` weighs the linework on outline and twotone.
- Everything else (`class`, `style`, `id`, `@click`, `data-*`, aria attributes) falls through to the `<svg>`.
...
Plain ESM with no build step to configure: the components are built with `h()`, so there is no SFC to compile.
